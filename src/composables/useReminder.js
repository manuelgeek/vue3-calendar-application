...
export function useReminder(daysData) {
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    const data = {
      ...rest,
      ...{ datetime, time, id: Math.random() * 1000, name: reminder },
    };
  ...
  const { errorMessage: reminderError, value: reminderValue } = useField(
    "reminder",
    yup.string().required().max(30)
  );
  const { errorMessage: dateTimeError, value: dateTimeValue } = useField(
    "dateTime",
    yup.string().required()
  );
  const { errorMessage: colorError, value: colorValue } = useField(
    "color",
    yup.string().required()
  );
  const { errorMessage: cityError, value: cityValue } = useField(
    "city",
    yup.string().required()
  );
  ...
  const form = reactive({
    ...
    date: {
      dateTimeError,
      dateTimeValue: dateTimeValue == null ? "" : dateTimeValue,
    },
  ...
  const getWeatherData = async () => {
    ...
    fetch(
      `https://api.openweathermap.org/data/2.5/forecast/daily?lat=37.5&lon=34.5&appid=${API_KEY}`
    )
