...
<template>
  <div :key="keyId" class="lg:flex lg:h-full lg:flex-col">
    ...
    <div
      v-if="'events' in selectedDay && selectedDay.events.length > 0"
    ...
    >
      ...
      >
        <li
          v-for="event in selectedDay.events.sort(
            (a, b) =>
              new Date(a.datetime + ' ' + a.time) -
              new Date(b.datetime + ' ' + b.time)
          )"
          :key="event.id"
          class="group flex p-4 pr-6 focus-within:bg-gray-50 hover:bg-gray-50"
        >
          <div class="flex-auto">
            <p :class="['font-semibold text-gray-900', event.color]">
              {{ event.name }}
            </p>
            <time
...
const { selectedDate, selectDate, today, showDayEvents, selectedDay } =
  useCalendar();
